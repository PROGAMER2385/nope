...
const listener = app.listen(settings.website.port, function() {
  console.log(chalk.green("[RELIACTYL] The dashboard has successfully loaded on port " + listener.address().port + "."));
});
  ...
  if (theme.settings.mustbeadmin.includes(req._parsedUrl.pathname)) {
    ...
    async function (err, str) {
      ...
      if (!req.session.userinfo || !req.session.pterodactyl) {
        if (err) {
          console.log(chalk.red(`[RELIACTYL] An error has occured on path ${req._parsedUrl.pathname}:`));
          console.log(err);
      ...
      function (err, str) {
        ...
        delete req.session.password;
        if (err) {
          console.log(`[RELIACTYL] An error has occured on path ${req._parsedUrl.pathname}:`);
          console.log(err);
          return res.send("An error has occured while attempting to load this page. Please contact an administrator to fix this.");
        };
        res.status(404);
